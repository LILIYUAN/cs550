...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, OURNFS, OURNFSVERS, ournfs_1, 0)) {
			_msgout("unable to register (OURNFS, OURNFSVERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
	...
	if (!svc_create(ournfs_1, OURNFS, OURNFSVERS, "netpath")) {
		_msgout("unable to create (OURNFS, OURNFSVERS) for netpath.");
		exit(1);
	}
